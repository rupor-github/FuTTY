...
const static struct ssh_mac *macs[] = {
    &ssh_hmac_sha2_256, &ssh_hmac_sha1, &ssh_hmac_sha1_96, &ssh_hmac_md5
};
...
static void ssh_logevent(Ssh ssh, const char *fmt, ...)
{
	...
	if (ssh->pkt_actx == SSH2_PKTCTX_NOAUTH && ssh->state != SSH_STATE_SESSION && !conf_get_int(ssh->conf, CONF_logomitdata)) {
